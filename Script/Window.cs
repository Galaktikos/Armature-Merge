...
namespace Galaktikos.ArmatureMerge
{
	public class Window : EditorWindow
	{
			...
			if (GUILayout.Button("Merge"))
			{
				...
				foreach (SkinnedMeshRenderer mergeMesh in MergeMeshes)
				{
					...
					for (int boneIndex = 0; boneIndex < mergeBones.Length; boneIndex++)
					{
						...
						if (IgnoreBonePath)
							found = FindChildRecursive(MainArmatureRoot, bone.name);
						else
						{
							string armaturePath = GetPathToArmature(bone, MergeArmatureRoot);
							if (armaturePath != null)
								found = MainArmatureRoot.Find(armaturePath);
						}
		...
		private Transform FindMergeMeshRoot(SkinnedMeshRenderer mergeMesh)
		{
			if (mergeMesh.rootBone == null)
				return null;

			if (IgnoreBonePath)
				return FindChildRecursive(MainArmatureRoot, mergeMesh.rootBone.name);

			string armaturePath = GetPathToArmature(mergeMesh.rootBone, MergeArmatureRoot);
			return armaturePath == null ? null : MainArmatureRoot.Find(armaturePath);
		}
